...
public class TestConfig implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		/*
		 * *
		 * User user1 = userService.register(new User("Joao da Silva", "jose@gmail.com",
		 * "1234", Roles.USER));
		 * User user2 = userService.register(new User("Juca Bala", "juca@gmail.com",
		 * "1234", Roles.USER));
		 * User admin = userService.register(new User("Admin Master",
		 * "admin@reddit.com", "admin123", Roles.ADMIN));
		 * User troll = userService.register(new User("Hater", "troll@gmail.com",
		 * "1234", Roles.USER));
		 * User userToDelete = userService.register(new User("Temp User",
		 * "temp@gmail.com", "1234", Roles.USER));
		 */
		 ...
		 * // ==========================================
		 * Forum forum1 = forumService.createForum("cars", user1.getId(),
		 * "All about cars");
		 * Forum forum2 = forumService.createForum("trucks", user1.getId(),
		 * "Big trucks community");
		 * Forum techForum = forumService.createForum("programming", admin.getId(),
		 * "Software Engineering discussions");
		 * 
		 * forumService.editName(user1.getId(), forum1.getId(), "automobiles");
		 * forumService.editDescription(user1.getId(), forum1.getId(),
		 * "Updated description: All about cars and vehicles");
		 * 
		 ...
		 * // 3. Teste PostService
		 * // ==========================================
		 * Post post1 = postService.addPost(user1.getId(), forum1.getId(),
		 * "I like cars", "Cars are really cool.");
		 * Post post2 = postService.addPost(admin.getId(), techForum.getId(),
		 * "Java 21 Released!", "Let's discuss.");
		 * Post postTroll = postService.addPost(troll.getId(), techForum.getId(),
		 * "Java is terrible", "Worst language.");
		 * 
		 * postService.editTitle(user1.getId(), post1.getId(), "I love JDM cars");
		 * postService.editContent(user1.getId(), post1.getId(),
		 * "Content updated with details about Honda and Toyota.");
		 * 
		 ...
		 * // ==========================================
		 * Comment c1 = commentService.addComment(user2.getId(),
		 * "I agree, cars are amazing!", post1.getId());
		 * Comment c2 = commentService.addComment(user1.getId(),
		 * "Java 21 Virtual Threads are game changers!", post2.getId());
		 * 
		 * Comment reply = commentService.addReply(c2.getId(), user2.getId(),
		 * "Totally! Thread performance is awesome now.",
		 * post2.getId());
		 * 
		 * commentService.editComment(user2.getId(), c1.getId(),
		 * "I totally agree! Japanese cars are super reliable.");
